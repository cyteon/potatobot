...
def apply_context_errors(embed, context, ignore_message=False):
    embed.add_field(name="Author", value=f"{context.author.mention}", inline=True)
    ...
    if context.command:
        embed.add_field(name="Command", value=f"`{context.command.name}`", inline=True)

    if context.message.content != "" and not ignore_message:
        embed.add_field(
            name="Message", value=f"```{context.message.content}```", inline=True
        )
    ...
    if context.interaction:
        ...
        embed.add_field(
            name="Interaction Options", value=f"```{options}```", inline=True
        )


async def command_error(error, context):
    async with aiohttp.ClientSession() as session:
        command_error_webhook = Webhook.from_url(
            config["command_error_webhook"], session=session
        )
        ...
        await command_error_webhook.send(
            embed=embed, username="PotatoBot - Error Logger"
        )
