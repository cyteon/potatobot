...
class CodeModal(ui.Modal, title="Run Code"):
    language = ui.TextInput(
        label="Language",
        placeholder="Enter the language of your code",
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        async with aiohttp.ClientSession() as session:
            embed = discord.Embed(
                title=f"Running your {self.language.value} code...", color=0xFFFFFF
            )
            ...
            code += shortened * "\n\n**Code shortened**"
            embed.add_field(
                name="Code", value=f"```{self.language.value}\n{code}```", inline=False
            )
            ...
            embed = discord.Embed(
                title=f"Ran your {json['language']} code", color=0xFFFFFF
            )
