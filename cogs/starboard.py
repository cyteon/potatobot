...
class Starboard(commands.Cog, name="⭐ Starboard"):
    ...
    async def on_raw_reaction_add(self, payload) -> None:
        ...
        starboard_message = await CachedDB.find_one(
            starboard_col, {"message_id": message.id}
        )
            ...
            if star_reactions >= guild["starboard"]["threshold"]:
                if not starboard_message:
                    ...
                    msg = await starboard_channel.send(
                        label, embed=embed, view=JumpToMessageView(message)
                    )

                    newdata = {"message_id": message.id, "starboard_id": msg.id}
                    ...
                    starboard_message = await starboard_channel.fetch_message(
                        starboard_message["starboard_id"]
                    )

                    await starboard_message.edit(
                        content=label, embed=embed, view=JumpToMessageView(message)
                    )
    ...
    async def starboard(self, context: Context) -> None:
        ...
        for subcommand in subcommands:
            ...
            data.append(
                f"{await self.bot.get_prefix(context)}starboard {subcommand.name} - {description}"
            )

        help_text = "\n".join(data)
        embed = discord.Embed(
            title=f"Help: Starboard",
    ...
    async def set_starboard(
        ...
        newdata = {"$set": {"starboard.channel": channel.id}}
    ...
    async def set_threshold(self, context: Context, threshold: int) -> None:
        ...
        newdata = {"$set": {"starboard.threshold": threshold}}

        await CachedDB.update_one(col, {"id": context.guild.id}, newdata)
        await context.send(f"Starboard threshold set to {threshold}.")

    @starboard.command(
        name="disable", description="Disable the starboard.", usage="starboard disable"
    )
    @commands.check(Checks.is_not_blacklisted)
    @commands.check(Checks.command_not_disabled)
    @commands.has_permissions(manage_channels=True)
    ...
    @starboard.command(
        name="enable", description="Enable the starboard.", usage="starboard enable"
    )
    @commands.check(Checks.is_not_blacklisted)
    @commands.check(Checks.command_not_disabled)
    @commands.has_permissions(manage_channels=True)
